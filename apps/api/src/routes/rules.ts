...
router.get('/', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const rules = await prisma.userRule.findMany({ where: { userId: req.user!.userId }, orderBy: { priority: 'desc' } });
...
router.put('/:id', validateBody({}), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const result = await prisma.userRule.updateMany({ where: { id: req.params.id, userId: req.user!.userId }, data });
    if (result.count === 0) return res.status(404).json({ success: false, error: 'Rule not found' });
    const rule = await prisma.userRule.findFirst({ where: { id: req.params.id, userId: req.user!.userId } });
...
router.delete('/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const result = await prisma.userRule.deleteMany({ where: { id: req.params.id, userId: req.user!.userId } });
    if (result.count === 0) return res.status(404).json({ success: false, error: 'Rule not found' });
