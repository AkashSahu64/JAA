...
const leverFieldSelector = 'input[name], input[id], textarea[name], textarea[id], select[name], select[id]';

type FieldKind = 'TEXT' | 'TEXTAREA' | 'SELECT' | 'CHECKBOX' | 'FILE' | 'HIDDEN';
...
export class LeverPlaywrightFormPort implements LeverFormPort {
  ...
  async snapshot(): Promise<LeverFormSnapshot> {
    ...
    return { step: await this.currentStep(), fields, hasNextStep: await this.nextStepButton().count() > 0 };
  }
  ...
  async select(fieldId: string, value: string): Promise<void> {
    await this.control(fieldId).selectOption({ label: value }).catch(async () => this.control(fieldId).selectOption(value));
  }
