    ...
    <div className="space-y-6">
      ...
      {!loading && !jobError && filteredJobs.length > 0 && <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5">{selectedJob && <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 sticky top-24">
          ...
          {selectedJob.applicationStatus ? <div className="py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs text-center">Application status: {selectedJob.applicationStatus}</div> : selectedJob.applicationUrl ? <a href={selectedJob.applicationUrl} target="_blank" rel="noreferrer" className="w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex justify-center gap-2">Open employer application <ExternalLink className="w-4 h-4" /></a> : <div className="py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-400 text-xs text-center">Application link unavailable. In-app submission is not implemented.</div>}
