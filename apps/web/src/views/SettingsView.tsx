...
import { createRule, createSearchProfile, deleteRule, deleteSearchProfile, fetchEmailConnections, fetchRules, fetchSearchProfiles, revokeEmailConnection, setSearchProfileActive, startEmailOAuth, syncEmailConnection, UIRule } from '../services/api';
...
export const SettingsView: React.FC = () => {
  ...
  const [emailProvider, setEmailProvider] = useState<'GMAIL' | 'MICROSOFT_GRAPH'>('GMAIL');
  ...
  const revokeConnection = async (connection: UIEmailConnection) => {
    setEmailBusy(connection.id); setError(''); setEmailMessage('');
  ...
  const syncConnection = async (connection: UIEmailConnection) => {
    setEmailBusy(connection.id); setError(''); setEmailMessage('');
    try { const queued = await syncEmailConnection(connection.id); setEmailMessage(`${connection.provider} sync ${queued.replayed ? 'already queued' : 'queued'} for ${connection.accountLabel}.`); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not queue email synchronization.'); }
    finally { setEmailBusy(null); }
  };
  const connectEmail = async (event: FormEvent) => {
    event.preventDefault();
    if (!emailAccount.trim()) return;
    setEmailBusy('connect'); setError(''); setEmailMessage('');
    try { const result = await startEmailOAuth(emailProvider, emailAccount.trim(), emailProvider === 'GMAIL' ? ['https://www.googleapis.com/auth/gmail.readonly'] : ['Mail.Read'], `${window.location.origin}/api/email-connections/oauth/callback`); window.location.assign(result.authorizationUrl); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not start mailbox authorization.'); setEmailBusy(null); }
  };
  ...
  return <div className="space-y-6">
    ...
    <section className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-3"><h2 className="font-bold text-sm text-white flex gap-2"><ShieldCheck className="w-4 h-4 text-emerald-400" />Credential boundaries</h2><p className="text-sm text-slate-400">AI and encryption secrets remain server environment variables and are never returned to this browser. Provider credentials use the durable encrypted server-side vault with owner-scoped access and revocation; the browser never receives token material. Configure production secrets through your deployment platform.</p></section>
    <section className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-4"><h2 className="font-bold text-sm text-white">Email consent</h2><p className="text-xs text-slate-400">Mailbox access uses durable server-side consent and encrypted credentials. The browser never receives provider tokens.</p><form onSubmit={connectEmail} className="grid sm:grid-cols-[12rem_1fr_auto] gap-2"><select value={emailProvider} onChange={(event) => setEmailProvider(event.target.value as typeof emailProvider)} aria-label="Email provider" className="rounded-lg bg-slate-950 border border-slate-800 px-2 py-1.5 text-xs text-white"><option value="GMAIL">Gmail</option><option value="MICROSOFT_GRAPH">Microsoft Graph</option></select><input required maxLength={320} value={emailAccount} onChange={(event) => setEmailAccount(event.target.value)} placeholder="Mailbox account label" aria-label="Mailbox account label" className="rounded-lg bg-slate-950 border border-slate-800 px-2 py-1.5 text-xs text-white" /><button type="submit" disabled={emailBusy !== null} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-[11px] font-semibold text-white disabled:opacity-50">{emailBusy === 'connect' ? 'Opening…' : 'Connect mailbox'}</button></form>{emailMessage && <p role="status" className="text-xs text-emerald-300">{emailMessage}</p>}{emailConnections.length === 0 ? <p className="text-xs text-slate-500">No email connections granted.</p> : <div className="space-y-2">{emailConnections.map((connection) => <div key={connection.id} className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between gap-3"><div><p className="font-bold text-sm text-white">{connection.provider} · {connection.accountLabel}</p><p className="text-[11px] text-slate-500">{connection.status} · {connection.scopes.join(', ') || 'No scopes'}{connection.lastSyncAt ? ` · Last sync ${new Date(connection.lastSyncAt).toLocaleString()}` : ''}</p></div>{connection.status === 'ACTIVE' && <div className="flex gap-2">{(connection.provider === 'GMAIL' || connection.provider === 'MICROSOFT_GRAPH') && <button type="button" disabled={emailBusy === connection.id} onClick={() => void syncConnection(connection)} className="rounded-lg bg-indigo-600 px-3 py-1.5 text-[11px] font-semibold text-white disabled:opacity-50">{emailBusy === connection.id ? 'Syncing…' : 'Sync now'}</button>}<button type="button" disabled={emailBusy === connection.id} onClick={() => void revokeConnection(connection)} className="rounded-lg bg-rose-600 px-3 py-1.5 text-[11px] font-semibold text-white disabled:opacity-50">{emailBusy === connection.id ? 'Working…' : 'Revoke'}</button></div>}</div>)}</div>}</section>
    <section className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
      ...
      <form onSubmit={addProfile} className="grid sm:grid-cols-2 gap-2">
        ...
        <input required type="number" min={1} max={10000} step={1} value={profileDailyLimit} onChange={(event) => setProfileDailyLimit(event.target.value)} placeholder="50" aria-label="Daily application limit" className="rounded-lg bg-slate-950 border border-slate-800 px-2 py-1.5 text-xs text-white" />
